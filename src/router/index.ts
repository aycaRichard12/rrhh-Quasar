  ...
  Router.beforeEach((to) => {
    ...
    if (authDataString) {
      ...
      try {
        ...
        if (authData && Array.isArray(authData.menu)) {
          // FUNCIÓN RECURSIVA SENIOR: Valida la existencia de un código en los 3 niveles
          const verificarCodigoPermiso = (nodos: MenuNodo[], codigoBuscado: string): boolean => {
            for (const nodo of nodos) {
              if (nodo.codigo === codigoBuscado) {
                return true;
              }
              if (nodo.submenu && Array.isArray(nodo.submenu) && nodo.submenu.length > 0) {
                if (verificarCodigoPermiso(nodo.submenu, codigoBuscado)) {
                  return true;
                }
              }
            }
          ...
          // Buscamos si el usuario tiene habilitado el módulo o pestaña 'dashboard'
          hasDashboard = verificarCodigoPermiso(authData.menu, 'dashboard');
        }
      } catch (e) {
        console.error('Error parsing auth data in router guard:', e);
      }
