...
export function useEntesReguladores() {
  ...
  const idEmpresaActiva = ref('1'); // Deberías obtenerlo de tu store de autenticación/usuario
  ...
  const cargarEntes = async () => {
    cargando.value = true;
    try {
      listaEntes.value = await entesReguladoresService.obtenerLista(idEmpresaActiva.value);
    } catch (error) {
  ...
  const guardarEnte = async () => {
    try {
      const respuesta = await entesReguladoresService.guardar(enteActual.value, esEdicion.value, idEmpresaActiva.value);
  ...
  const confirmarEliminacion = (id: string) => {
    $q.dialog({
      ...
      try {
        await entesReguladoresService.eliminar(id);
  ...
  const procesarTemplates = (tipo: number) => {
    ...
    $q.dialog({
      title: '¿Está seguro?', message, ok: { color: 'primary', label: 'Proceder' }, cancel: true
    }).onOk(async () => {
      try {
        await entesReguladoresService.importarTemplates(tipo, listaTemplates.value, idEmpresaActiva.value);
