...
import { obtenerColumnasMetodosDePago } from '../utils/metodosDePago.columns';

const { t } = useI18n();
const listaColumnas = computed(() => obtenerColumnasMetodosDePago(t));
const filasTipadas = computed(() => datosFiltrados.value as unknown as FilaBase[]);

const props = defineProps<{
  listaMetodosDePago: MetodoDePago[];
  cargando: boolean;
  filtro: string;
}>();
...
const emits = defineEmits<{
  ...
  (e: 'cambiarEstadoRegistro', ente: MetodoDePago): void;
...
const emitirCambioEstado = (ente: MetodoDePago): void => {
...
} = useFiltroExcel(() => props.listaMetodosDePago, configuracionFiltros);
</script>