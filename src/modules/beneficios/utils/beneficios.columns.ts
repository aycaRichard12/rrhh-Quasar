...
export const obtenerColumnasBeneficios = (t: (key: string) => string): QTableColumn<Beneficio>[] => [
  ...
  { name: 'cantidad',   label: t('tables.amount'),     align: 'right', field: 'cantidad',   style: 'width: 80px', format: (val: number, row: Beneficio) => (row.tipo === 1 ? `${val} %` : `${val} Bs.`)},
  { name: 'orden',      label: t('tables.order'),      align: 'center',field: 'orden',      style: 'width: 80px'},
  { name: 'destino',    label: t('tables.destination'),align: 'center',field: 'destino',    style: 'width: 100px', format: (val: number) => (val === 1 ? 'Planilla' : 'Finiquito')},
...
export const obtenerColumnasBeneficiosEstandar = (t: (key: string) => string): QTableColumn<Beneficio>[] => [
  { name: 'numero',     label: 'N°',                   align: 'right', field: () => '',     style: 'width: 50px' },
  { name: 'nombre',     label: t('beneficios.name'),   align: 'left',  field: 'nombre',     style: 'white-space: normal; width: 180px' },
  { name: 'descripcion',label: t('tables.description'),align: 'left',  field: 'descripcion',style: 'white-space: normal' },
  { name: 'tipo',       label: t('tables.type'),       align: 'center',field: 'tipo',       style: 'width: 100px', format: (val: number) => (val === 1 ? 'Porcentaje' : 'Monto')},
  { name: 'cantidad',   label: t('tables.amount'),     align: 'right', field: 'cantidad',   style: 'width: 80px', format: (val: number, row: Beneficio) => (row.tipo === 1 ? `${val} %` : `${val} Bs.`) },
  { name: 'orden',      label: t('tables.order'),      align: 'center',field: 'orden',      style: 'width: 80px' },
  { name: 'destino',    label: t('tables.destination'),align: 'center',field: 'destino',    style: 'width: 100px', format: (val: number) => (val === 1 ? 'Planilla' : 'Finiquito')}
];