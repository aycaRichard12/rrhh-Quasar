...
import { obtenerColumnasSalariosMinimos } from '../utils/salarioMinimo.columns';

const { t } = useI18n();
const listaColumnas = computed(() => obtenerColumnasSalariosMinimos(t));
const filasTipadas = computed(() => datosFiltrados.value as unknown as FilaBase[]);

const props = defineProps<{
  listaSalariosMinimos: SalarioMinimo[];
  cargando: boolean;
  filtro: string;
}>();
...
const emits = defineEmits<{
  ...
  (e: 'cambiarEstadoRegistro', salarioMinimo: SalarioMinimo): void;
...
const emitirCambioEstado = (salarioMinimo: SalarioMinimo): void => {
  emits('cambiarEstadoRegistro', salarioMinimo);
};

const { 
  filtrosActivos, valoresUnicosPorColumna, datosFiltrados,
  orden, establecerOrden, limpiarFiltrosColumna
} = useFiltroExcel(() => props.listaSalariosMinimos, configuracionFiltros);
</script>